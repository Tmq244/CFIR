...
def main(args):
   ...
   # print type(score)
    ...
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        ...
        print('Loaded checkpoint: epoch={} metric={} value={}'.format(
            checkpoint.get('epoch'),
            checkpoint.get('metric_name'),
    ...
    for SPLIT in SPLITS:
        ...
        for target in targets:
            ...
            mrr+=_pos
        ntargets = float(len(targets)) if targets else 1.0
        r1 /= ntargets
        r5 /= ntargets
        r8 /= ntargets
        r10 /= ntargets
        ...
        output_path = os.path.join(args.output_root, '{}_{}'.format(date_key, args.expr_name))
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        with open(os.path.join(output_path,'hyperopt.{}.pkl'.format(SPLIT)),'wb') as f:
            pickle.dump(hyperopt, f)
        with open(os.path.join(output_path,'score.{}.pkl'.format(SPLIT)),'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, required=True, help='Path to model checkpoint (.pkl/.pth)')
    parser.add_argument('--output_root', type=str, default='./output_score', help='Directory to store offline score outputs')
